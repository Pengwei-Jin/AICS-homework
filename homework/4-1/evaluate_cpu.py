...
def net(data_path, input_image):
    ...
    for i, name in enumerate(layers):
        if name[:4] == 'conv':
            ...
            kernels, bias = weights[i][0][0][0][0]
            current = tf.nn.bias_add(tf.matmul(tf.reshape(current, [current.shape[0],-1]), tf.reshape(kernels, [-1,kernels.shape[-1]])), bias.squeeze()) 
        elif name  == 'fc7':
            kernels, bias = weights[i][0][0][0][0]
            current = tf.nn.bias_add(tf.matmul(current, tf.reshape(kernels, [-1,kernels.shape[-1]])), bias.squeeze()) 
        elif name  == 'fc8':
            kernels, bias = weights[i][0][0][0][0]
            current = tf.nn.bias_add(tf.matmul(current, tf.reshape(kernels, [-1,kernels.shape[-1]])), bias.squeeze()) 
...
def _conv_layer(input, weights, bias):
    ...
    #x = tf.nn.conv2d(input, filters=tf.convert_to_tensor(weights), strides=[1,1,1,1], padding='SAME')
    #return tf.nn.bias_add(x, bias[0], data_format='NHWC')
    return tf.nn.bias_add(tf.nn.conv2d(input, filter=weights, padding='SAME', use_cudnn_on_gpu=False), bias.squeeze())
...
def load_image(path):
    ...
    image = scipy.misc.imresize(scipy.misc.imread(path), size=(224,224))
